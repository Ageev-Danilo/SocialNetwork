...
import { MOCK_ALBUMS, THEMES, YEARS } from '@/modules/albums/albums.mock';
...
export default function EditAlbumScreen() {
    ...
    const [theme,     setTheme]     = useState(found?.theme ?? '');
    const [year,      setYear]      = useState(found?.year  ?? '');
    ...
    return (
        ...
        >
            <View style={styles.modal}>
                ...
                <View style={[BASE.yc, { justifyContent: 'flex-end', gap: 10, marginTop: 16 }]}>
                    <Button type="outline" text="Скасувати" onPress={() => router.back()} />
                    <Button type="fill"    text="Зберегти"  onPress={handleSave} />
                </View>
            </View>
        </ScrollView>
    );
